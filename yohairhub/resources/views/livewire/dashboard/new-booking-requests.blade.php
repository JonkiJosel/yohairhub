<div class="p-4 bg-white rounded shadow" wire:poll>
    <div class="flex flex-col md:flex-row justify-between">
        <div>
            <h2 class="text-xl font-semibold mb-4">
                New Booking Requests: {{ $showDaily ? 'Today' : 'This Week' }} ({{ now()->format('Y-m-d') }})
            </h2>
        </div>
                ...
                @foreach ($bookings as $booking)
                    <tr>
                        <td class="px-2 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-900">{{ $booking->reservation_date }}</div>
    ...
    @if ($bookings->isEmpty())
        <div class="mt-4 text-gray-500 text-sm">No new booking requests.</div>
    @endif
</div>
